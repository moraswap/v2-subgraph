...
import { Bundle, Pair, PairDayData, Token, TokenDayData, MoraswapDayData, MoraswapFactory } from '../types/schema'
import { PairHourData } from './../types/schema'
import { ONE_BI, ZERO_BD, ZERO_BI } from './helpers'
import { FACTORY_ADDRESS } from '../constants'

export function updateMoraswapDayData(event: EthereumEvent): MoraswapDayData {
  let moraswap = MoraswapFactory.load(FACTORY_ADDRESS)
  let timestamp = event.block.timestamp.toI32()
  let dayID = timestamp / 86400
  let dayStartTimestamp = dayID * 86400
  let moraswapDayData = MoraswapDayData.load(dayID.toString())
  if (moraswapDayData === null) {
  ...
  return moraswapDayData as MoraswapDayData
}
